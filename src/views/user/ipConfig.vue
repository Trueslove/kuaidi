    ...
    <el-dialog :title="dialogTitle" :visible.sync="isShowDialog" width="25%">
      <el-form :model="diaForm" :rules="rules" ref="diaForm">
        ...
        <el-form-item label="ip地址" prop="address">
          <el-input v-model="diaForm.address" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
import { ipAddApi, ipGetApi, ipDelApi } from "@s/api";
export default {
  mixins: [mixinTable],
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请填写ip名称", trigger: "blur" }],
        address: [{ required: true, message: "请填写ip地址", trigger: "blur" }],
      },
    };
  },
  created() {
    this.tableItem = [
      { prop: "name", label: "ip名称" },
      { prop: "address", label: "ip地址" },
  ...
  methods: {
    ...
    handleConfirm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.isEdit) {
            ipAddApi({ ...this.diaForm }).then((res) => {
              this.$message({
                message: res.message,
                type: "success",
              });
    ...
    handleDelete(row) {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          ipDelApi({ ipid: row.ipid }).then((res) => {
            this.$message({
              type: "success",
              message: res.message,
            });
    ...
    getDataList() {
      // 获取列表数据
      ipGetApi({ ...this.searchData, ...this.pageParams }).then((res) => {
